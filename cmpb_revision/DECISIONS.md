...
| D-M4 | IBS split | **5-fold patient-level CV**, inner val 15%; data = flattened `franchisn/ibs-dataset` (`IBS-patient-dataset`) with bundled `ibs/metadata/ibs_groups.csv` (**126** exam groups: 16 IBS + 110 Normal) | 2026-09-03 | Numeric `pre-processed-ibs` dump has no exam IDs and cannot be fully remapped (pHash/ORB). `Normal_1/2/3` are upload shards, not clinical subtypes. `extract_group_id` recognizes `Proc…_s_f.JPG`. |
