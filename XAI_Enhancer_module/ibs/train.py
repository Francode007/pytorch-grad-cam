"""
IBS pre-processed dataset training script with 80:20 train/val, optimized batching, and standard args.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train IBS classifier")
    p.add_argument("--data-root", type=str, default="data/IBS-preprocessed-dataset",
                    help="IBS dataset root (containing IBS/ and Normal/ folders)")
    ...
    p.add_argument("--pin-memory", action="store_true", default=True,
                    help="Pin memory for DataLoader (disable for MPS if you see errors)")
    ...
    p.add_argument("--amp", action="store_true", default=False,
                    help="Use automatic mixed precision (recommended for A100)")
    p.add_argument("--amp-dtype", type=str, default="float16", choices=["float16", "bfloat16"],
                    help="AMP dtype (bfloat16 often better on A100)")
    p.add_argument("--compile", action="store_true", default=False,
                    help="Use torch.compile(model) for speed (PyTorch 2+, CUDA)")
    ...
    p.add_argument("--a100", action="store_true",
                    help="Preset for A100 40GB: batch_size=128, amp, bfloat16, compile, num_workers=8")
    args = p.parse_args()
...
def create_loaders(args, data_root: Path):
    ...
    train_ds = IBSDataset(str(data_root), split="train", transform=train_tf, splits_dir=str(splits_dir))
    val_ds = IBSDataset(str(data_root), split="val", transform=val_tf, splits_dir=str(splits_dir))
