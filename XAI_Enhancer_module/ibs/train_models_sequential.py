"""
Train multiple IBS models sequentially on a single GPU (e.g. A100).
...
Example (A100 40GB, from repo root):
  python -m XAI_Enhancer_module.ibs.train_models_sequential \\
    ...
    --data-root data/IBS-preprocessed-dataset --output-dir /path/on/server/ibs_runs \\
    --a100
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train multiple IBS models sequentially. All checkpoints saved under --output-dir."
    )
    p.add_argument(
        "--archs",
        nargs="+",
    ...
    p.add_argument("--output-dir", type=str, default="runs/ibs", help="Base dir for checkpoints")
...
def main():
    ...
    train_module = "XAI_Enhancer_module.ibs.train"
