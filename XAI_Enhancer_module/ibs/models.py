"""
Build and load IBS classification models (ImageNet-pretrained backbone + IBS head).
"""
...
def build_ibs_model(
    arch: str,
    num_classes: int = IBS_NUM_CLASSES,
    pretrained: bool = True,
) -> nn.Module:
    """Build model for IBS: ImageNet backbone + linear head for num_classes."""
    arch = arch.lower()
...
def load_ibs_checkpoint(
    ...
    """
    Load checkpoint into model. Handles both raw state_dict and dict with
    'model_state_dict' / 'state_dict'. Strips torch.compile prefixes.
    """
