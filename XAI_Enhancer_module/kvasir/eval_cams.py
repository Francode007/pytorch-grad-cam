...
def _main_body(args):
    ...
    if "hrcam" in methods:
        ...
        from XAI_Enhancer_module.kvasir.data import KvasirDataset, KVASIR_NUM_CLASSES, get_train_transforms, get_val_transforms
        from torch.utils.data import DataLoader
        ...
        if args.hrcam_checkpoint:
            ...
            train_ds = KvasirDataset(str(args.data_root), split="train", transform=get_train_transforms())
            val_ds = KvasirDataset(str(args.data_root), split="val", transform=get_val_transforms())
