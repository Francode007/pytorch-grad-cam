...
class CausalMetric():
    ...
    def __init__(self, model, model_name, mode, step, substrate_fn):
...
def get_metrics(model,model_name, img_size=224):
    # substrate fn
    klen = 11
    ksig = 5
    ...
    insertion = CausalMetric(model, model_name, 'ins', img_size, substrate_fn = blur)
    deletion = CausalMetric(model, model_name, 'del', img_size, substrate_fn=torch.zeros_like)
